...
extern "C" {
    void Java_org_cocos2dx_cpp_Cocos2dxImagePicker_ImagePickerResult(JNIEnv *env, jobject thiz, jbyteArray array, jint orientation)
    {
        if(array != NULL){
            ...
            Director::getInstance()->getScheduler()->performFunctionInCocosThread([]{
				PluginManager::getInstance()->getImagePickerController()->finishImage(nullptr);
            });
        }
    }
}
...
void ImagePickerImpl::openImage()
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    JniMethodInfo t;
    bool result = JniHelper::getStaticMethodInfo(t, "org/cocos2dx/cpp/Cocos2dxImagePicker", "openImage", "()V");
    if (result)
    {
	...
	if(GetOpenFileName(&ofn)){
		...
		Image *image = new Image();
		image->initWithImageFile(std::string(temp));

		PluginManager::getInstance()->getImagePickerController()->finishImage(image);
	}
	else{
		PluginManager::getInstance()->getImagePickerController()->finishImage(nullptr);
	}
